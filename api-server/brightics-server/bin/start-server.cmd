...
SET GC_OPTS=-XX:+UseConcMarkSweepGC -verbose:gc -XX:+PrintGCTimeStamps -Xloggc:"%BRIGHTICS_SERVER_HOME%gc.out" -XX:MaxMetaspaceSize=512m -XX:+CMSClassUnloadingEnabled
SET JAVA_OPTS=-Xms1g -Xmx2g -XX:+HeapDumpOnOutOfMemoryError -Djava.net.preferIPv4Stack=true -Djava.io.tmpdir="%BRIGHTICS_SERVER_HOME%tmp" -Dbrightics.local.token=%ACCESS_TOKEN% -Dbrightics.local.user=%USER_ID% 
...
java %GC_OPTS% %JAVA_OPTS% -classpath . %MAIN% 

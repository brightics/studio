...
public class JobRunnerContext {
    ...
    private JobRunnerContext() {
        ...
        if (jobRunnerBuilder == null) {
            jobRunnerBuilder = JobRunnerBuilder.builder().config(config).jobRunnerApi(new DefaultJobRunnerApi());
        }
    ...
    public JobRunnerWrapper createJobRunner(String pythonProcessName, String modelFilePath) throws Exception {
        return createJobRunnerWithData(pythonProcessName, modelFilePath);
    }

    /**
     * Create JobRnnner which contain model and prepared data.
     *
     * @param pythonProcessName : Set the python process name to be executed.
     * @param modelFilePath     : runnable model json file path.
     * @param preparedDataVO    : prepared data.
     */
    public JobRunnerWrapper createJobRunnerWithData(String pythonProcessName, String modelFilePath,
                                                    PreparedDataVO... preparedDatas) throws Exception {
        String modelJsonString = FileUtils.readFileToString(new File(modelFilePath));
        return createJobRunnerWithJsonString(pythonProcessName, modelJsonString, preparedDatas);
    }
    ...
    private JobRunnerWrapper createJobRunnerWithJsonString(String pythonProcessName, String modelJsonString,
        ...
        if (preparedDatas != null) {
            jobParam.setDatas(Arrays.asList(preparedDatas));
        }

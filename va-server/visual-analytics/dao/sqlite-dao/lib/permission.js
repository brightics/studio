...
module.exports = {
    permission: {
        checkSchema: function (_errCallback, _doneCallback) {
            ...
            const batch = (records) => (r) => {
                if (r.length === 0) {
                    batchUpdate(PERMISSION_CREATE, records);
                }
                doneCallback(r.rows, r, DDL_CHECK_TABLE, ['brtc_permission']);
            };

            query(DDL_CHECK_TABLE, ['brtc_permission'], errCallback, function (result) {
                if (result.length === 0) {
                    onEmpty(errCallback, doneCallback, result);
                    return;
                }
                ...
                query('select * from brtc_permission where resource_type = \'datasource\'', [], errCallback, batch([
                    ['perm_datasource_create', 'datasource', 'Create a datasource'],
                    ['perm_datasource_read', 'datasource', 'Read a datasource'],
                    ['perm_datasource_update', 'datasource', 'Update a datasource'],
                    ['perm_datasource_delete', 'datasource', 'Delete a datasource']
                ]));

                query('select * from brtc_permission where resource_type = \'deploy\'', [], errCallback,
                    batch([
                        ['perm_deploy_create', 'deploy', 'Create a deploy'],
                        ['perm_deploy_read', 'deploy', 'Read a deploy'],
                        ['perm_deploy_update', 'deploy', 'Update a deploy'],
                        ['perm_deploy_delete', 'deploy', 'Delete a deploy']
                    ]));

                query('select * from brtc_permission where resource_type = \'publish\'', [], errCallback,
                    batch([
                        ['perm_publish_create', 'publish', 'Create a publish'],
                        ['perm_publish_read', 'publish', 'Read a publish'],
                        ['perm_publish_delete', 'publish', 'Delete a publish']
                    ]));

                query('select * from brtc_permission where permission_id in (\'perm_schedule_read_my\', \'perm_publish_read_my\')', [], errCallback,
                    batch([
                        ['perm_schedule_read_my', 'schedule', 'Create a my publish'],
                        ['perm_publish_read_my', 'publish', 'Read a my publish']
                    ]));
            });
        },

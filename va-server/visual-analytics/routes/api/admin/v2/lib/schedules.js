    ...
    return function (error, response, body) {
        if (error) {
            ...
            if (response.statusCode === 200) {
                if (parse) {
                    const ans = withResult ? JSON.parse(body).result : JSON.parse(body);
                    res.json(ans);
                } else {
...
var _executeInPermission = function (req, res, perm, task) {
    var permHandler = __BRTC_PERM_HELPER.checkPermission(req,
        [__BRTC_PERM_HELPER.PERMISSION_RESOURCE_TYPES.SCHEDULE], perm);
...
var getSchedule = function (req, res, next) {
    ...
    request(options, function (error, response, body) {
        ...
        const checkVersion = (schedule) => {
            ...
            var versionId = JSON.parse(schedule.executeContents)
                .models[JSON.parse(schedule.executeContents).main].version_id;
            ...
            return new Promise(function (resolve, reject) {
                ...
                request(options, function (error, response, body) {
                    ...
                    if (response.statusCode !== 200) {
                        return reject(new Error(__BRTC_ERROR_HANDLER
                            .parseError(body).errors[0].message));
                    }

;(function (window, $, undefined) { ;(function () {
    ...
    datepicker.prototype = {
        ...
        year: function (moveYear) {
        	var d = this.parsedDate,
        	o = this.opts;
    		this.date = new Date(d.year + moveYear, d.month, 1);
    		if (o.onChangeDecade) o.onChangeDecade(this.curDecade);
        },
...
;(function () {
    ...
    datepicker.Body.prototype = {
        ...
        _getCellContents: function (date, type) {
            ...
            if (parent.selectedDates.length == 0 && dp.isSame(currentDate, date, type)) classes += ' -current-';
...
;(function () {
    var template = '' +
        ...
        '<div class="datepicker--nav-action dropdown" data-action="headerYear"><label class="dropbtn" >#{year}</label>'+
    ...
    datepicker.Navigation.prototype = {
        ...
        _render: function () {
            var title = this._getTitle(this.d.currentDate), year = this._getYear(this.d.currentDate),
                html = dp.template(template, $.extend({title: title, year: year}, this.opts));
        ...
        _yearRender: function () {
        	var centerYear = this._getYear(this.d.currentDate);
            var tag = "";
            var cnt = 7;
            var half = Math.floor(cnt/2);
            
            $('.dropdown-content', this.d.$nav).children().remove(); //지우고 다시 selectbox를 그려준다.
            for(var i = 0; i<cnt; i++) {
				var currentYear = half - i;
				if(i < half) $('.dropdown-content', this.d.$nav).append('<a data-action="year">'+(centerYear-currentYear)+'</a>');
				else $('.dropdown-content', this.d.$nav).append('<a '+(currentYear == 0 ? 'class="active" ' : '') +' data-action="year">'+(centerYear-currentYear)+'</a>');
			}
        },
        ...
        _onClickNavButton: function (e) {
            ...
            if(action == null || action == 'label') return;

            if(action == "year"){
            	var moveYear = parseInt(e.target.innerText) - parseInt(this._getYear(this.d.currentDate)); 
            	this.d[action](moveYear);
            	$('.dropdown-content').removeClass('-active-');
            }else if(action == "headerYear"){

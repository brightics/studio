		...
		while ((line = lineBuffer.readLine()) != null) {
			if (lineCount % ParquetClient.LINES_PER_ONE_PARQUET_FILE == 0) {
				...
				writer = new CsvParquetWriterBuilder(new Path(outputDirectory, fileName), names, types)
						...
						.withRowGroupSize(ParquetClient.ROW_GROUP_SIZE_IN_BYTE).withDictionaryEncoding(false)
			...
			try {
				...
				BrighticsCoreException brighticsCoreException = new BrighticsCoreException("4420",
						String.valueOf(lineCount + 1), Arrays.toString(row));
				logger.error(brighticsCoreException.getMessage(), e);
				throw brighticsCoreException.addDetailMessage(ExceptionUtils.getStackTrace(e));
			}
		}
	}

...
public class JobStatusTracker {
	...
	private final Set<String> controlFunctionNames = new HashSet<>(
			Arrays.asList("WhileLoop", "ForLoop", "If", "Flow", "Opt"));
	...
	public synchronized void endModelWith(Status status) {
		if (!(status == Status.SUCCESS || status == Status.FAIL)) {
			throw new IllegalStateException("Model cannot end with status in " + status.toString());
		}
	...
	public synchronized void endFunctionWith(Status status) {
		...
		if (functionStack == null) {
			throw new IllegalArgumentException("No function is running.");
		}
	...
	private static class FunctionMap {
		...
		public JobFunctionStatusVO getOrCreate(String fid, String label, String functionName) {
			return Optional.ofNullable(functionMap.get(genKey(fid))).orElseGet(() -> {

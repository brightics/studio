...
public abstract class OptWorkflow extends SequentialWorkflow {
	...
	public void start(WorkflowContext context) {
		...
		String scriptOfCreateBroptJob = OptScriptGenerator.createBroptJob(optJobId, optVariables, objectiveSense, constraints, method,
		...
		while (!isComplete) {
			...
			String scriptOfUpdateJob = OptScriptGenerator.updateBroptJob(optJobId, objectiveValue);
			...
			// get is complete 
			String scriptOfIsComplete = OptScriptGenerator.isCompleteBroptJob(optJobId);
		...
		String scriptOfGetBestParameter = OptScriptGenerator.getBestParameterBroptJob(optJobId);
	...
	private String getObjectiveValue(VariableContext variableContext, JsonObject objective) {
		try {
			...
			throw (new BrighticsCoreException("4417")).addDetailMessage(e.getMessage());
		}
	}
	...
	private Object runPythonScriptTask(String script) {
		...
		try {
			...
			JobContextHolder.getJobRunnerAPI().stopTask(taskId, "Python", "python");
			throw new BrighticsCoreException("3101");

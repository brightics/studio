...
import com.samsung.sds.brightics.common.workflow.flowrunner.vo.JobFunctionStatusVO;
...
public class JobStatusTracker {
    ...
    public synchronized void endModelWith(Status status) {
    	if (!(status == Status.SUCCESS || status == Status.FAIL)) {
			throw new IllegalStateException("Function cannot end with status in " + status.toString());
		}
    ...
    public synchronized void startFunction(String fid, String label, String functionName) {
    	if (modelStack == null || modelStack.isEmpty()) {
			throw new IllegalArgumentException("No model is running.");
		}
    ...
    private static class FunctionMap {
        private final Map<String, JobFunctionStatusVO> functionSyncMap = Collections.synchronizedMap(new HashMap<>());
        ...
        public void removeAll(List<JobFunctionStatusVO> functions) {
            for (JobFunctionStatusVO function : functions) {
                functionSyncMap.remove(genKey(function.getFid()));
            }
        }

        public JobFunctionStatusVO getOrCreate(String fid, String label, String functionName) {
            return Optional
                    .ofNullable(functionSyncMap.get(genKey(fid)))
                    .orElseGet(() -> {
                        ...
                        functionSyncMap.put(genKey(fid), newStatus);
        ...
        public JobFunctionStatusVO get(String fid) {
            return functionSyncMap.get(genKey(fid));
        }
    }
}

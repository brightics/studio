...
class Numeric_Functions_test(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.df1 = pd.DataFrame({
            ...
            'sinval': np.sin([-np.pi/2, -np.pi/3, -np.pi/4, -np.pi/6, 0,
                              np.pi/6, np.pi/4, np.pi/3, np.pi/2]),
            'tanval': np.tan([-np.pi/2.2, -np.pi/3, -np.pi/4, -np.pi/6, 0,
                              np.pi/6, np.pi/4, np.pi/3, np.pi/2.2]),
    ...
    def test03(self):
        ...
        ref = pd.DataFrame({
            ...
            'cos(csangle)': np.cos([-np.pi/2, -np.pi/3, -np.pi/4, -np.pi/6,
    ...
    def test19(self):
        ...
        ref = pd.DataFrame({
            ...
            'sin(csangle)': np.sin([-np.pi/2, -np.pi/3, -np.pi/4, -np.pi/6,
                                    0, np.pi/6, np.pi/4, np.pi/3, np.pi/2])
        })
        table_cmp(sql, res, ref, check_row_order=self.check_row_order,
                  print_dfs=self.print_dfs)
    ...
    def test23(self):
        ...
        ref = pd.DataFrame({
            'csangle': [-np.pi/2, -np.pi/3, -np.pi/4, -np.pi/6, 0,
                        np.pi/6, np.pi/4, np.pi/3, np.pi/2],
            'round(csangle,1)': np.around([-np.pi/2, -np.pi/3, -np.pi/4, -np.pi/6, 0,
    ...
    def test26(self):
        ...
        res = self.brtcsql.execute_sql_query(sql)
        ref = pd.DataFrame({
            'TRUNCATE(csangle,2)': [-1.57, -1.04, -0.78, -0.52, 0, 0.52, 0.78, 1.04, 1.57]
        })
        table_cmp(sql, res, ref, check_row_order=self.check_row_order,
                  print_dfs=self.print_dfs)

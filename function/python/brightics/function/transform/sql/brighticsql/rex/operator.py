...
# TODO We need refactoring. Find the evaluation patterns such as 'if np.any(naidx):...'

# TODO SqlOperator instance have operands as its member variable.
#  Maybe we can move operands to RexCall and then use singleton instance of
#  a specific SqlOperator.

import numpy as np

from brighticsql.base import RexReturn, SqlOperator, fbase
from brighticsql.functions.builtin_functions import (
    binary_operators, numeric_functions, prefix_operators, postfix_operators,
    sqlin_operators, string_functions, truncate_)
from brighticsql.functions.userdefined_functions import userdefined_functions
from brighticsql.tableclass import QueryTable
from brighticsql.utils.dtypes import (infer_array_dtype, default_val,
                                      upcast)
    ...
    def eval_op(self, table, get_naidx=True, filter_index=False,
        ...
        for oprd in self.operands[1:]:
            ...
            if not isinstance(ret_dtype, np.dtype):
                ret_dtype = upcast([left.dtype, right.dtype])
...
class ArrayValueConstructor(SqlOperator):
    ...
    def eval_op(self, **kwargs):
        ...
        return RexReturn(np.array(elem), infer_array_dtype(elem), None)
    ...
    def __init__(self, kind, name, operands, sqlfunction_category, **kwargs):
        ...
        if self.sqlfunction_category == 'NUMERIC':
            if name == 'RAND' and len(operands) == 0:
                ...
                self.op = fbase('TRUNCATE', truncate_, 'one',

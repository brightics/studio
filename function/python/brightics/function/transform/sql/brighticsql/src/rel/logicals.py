...
from ..base import RelNodeABC, CorrvarSpace
from ..tableclass import NullTable, QueryTable, ModelOutput
from ..utils.group_util import sort_indexer
from ..utils.rand_util import gen_colname
from ..fieldgenerator import make_projectfield
...
class LogicalTableScan(RelNodeABC):
    def __init__(self, rel_typename, _id, inputs, field, varSet, **kwargs):
        self.table = kwargs.pop('table')
        super().__init__(rel_typename, _id, inputs, field, varSet, **kwargs)

    def __repr__(self):
        return f'{self.enum_name}(table=[[{self.table.schema}, {self.table.table}]])'

    def _eval(self, table_space, corrvar_space, **kwargs):
...
class LogicalValues(RelNodeABC):
    def __init__(self, rel_typename, _id, inputs, field, varSet, **kwargs):
        self.tuples = kwargs.pop('tuples')
        super().__init__(rel_typename, _id, inputs, field, varSet, **kwargs)

    def __repr__(self):
        s = self.tuples[0][0].value
        return f'{self.enum_name}' + f'(tuples=[[{{s}}]])'
...
class LogicalSort(RelNodeABC):
    ...
    def __repr__(self):
        ...
        if self.collation.fieldCollations:
            ...
            s.append(', '.join(
                [f'sort{i}=[${fc.field_index}-{fc.null_direction}-{fc.direction}]' for i, fc in g]))
...
class LogicalProject(RelNodeABC):
    def __init__(self, rel_typename, _id, inputs, field, varSet, **kwargs):
    ...
    def is_trivial_project(self, input_field):
        ...
        if len(input_field) == len(self.exps):
            from_rex = (exp.index if exp.rex_typename ==
                        'RexInputRef' else -1 for exp in self.exps)
                ...
                for ehs, e in zip(self.exps_has_subq, self.exps):
                    if ehs in {'N', 'S'}:
                        ...
                        if len_cnms == 1:
                            ...
                            # g = itertools.product(table.get_rowiter(),
                            #                       repeat=len_cnms)
                            # def f():
                            #     for item in g:
                            #         for cnm, it in zip(cnames, item):
                            #             cvspace.space[cnm] = it
                            #         yield e(**_kw).value
                            g = table.rowiter()
...
class LogicalFilter(RelNodeABC):
    def __init__(self, rel_typename, _id, inputs, field, varSet, **kwargs):
...
class LogicalAggregate(RelNodeABC):
    def __init__(self, rel_typename, _id, inputs, field, varSet, **kwargs):
    ...
    def __repr__(self):
        s = ", ".join([str(i) for i in self.groupset])
        s = [f'group=[{{s}}]']
        if self.aggcalls:
        ...
        if self.groupset and not self.aggcalls:
            ...
            def g():
                for agg in self.aggcalls:
                    idx = agg.arglist[0] if agg.arglist else 0
                    yield agg(table.slice_col(idx), table.get_dtypes(idx))
...
class LogicalJoin(RelNodeABC):
    ...
    def __repr__(self):
        return f'{self.enum_name}(condition=[{self.condition}], joinType=[{self.jointype}])'
    ...
    def _crossjoin(self, left, right, field):
        idxL, idxR = np.broadcast_arrays(
            *np.ogrid[:left.shape[0], :right.shape[0]])
        new_dtypes = left.get_dtypes() + right.get_dtypes()
        if left.dstruct == pd.DataFrame:
            left = left.data.values[idxL.ravel()]
        else:
            left.to_ndarray()
            left = left.data[idxL.ravel()]
        if right.dstruct == pd.DataFrame:
            right = right.data.values[idxR.ravel()]
        else:
            right.to_ndarray()
            right = right.data[idxR.ravel()]
    ...
    def _simple_eqcond_join(self, left, right, fields, jointype,
                            simple_colref):
    ...
    def _conditional_join(self, left, right, field, jointype, condition):
        # TODO improve this method.
        # this method is memory inefficient
        # currently supports inner join only
        if jointype == 'INNER':
            _table = self._crossjoin(left, right, field)
            fields_joined = _table.fields
            _table = _table.toDataFrame()
            condition_exp = make_projectfield(self.condition, fields_joined)
        else:
    ...
    def __call__(self, table_space, corrvar_space, *args, **kwargs):
        ...
        if self.is_crossjoin:
            ...
            if simple_colref:
                joined = self._simple_eqcond_join(
                    left, right, field, jointype, simple_colref)
            else:
                joined = self._conditional_join(

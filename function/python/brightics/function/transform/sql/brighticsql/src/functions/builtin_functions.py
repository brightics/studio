...
numeric_functions['TRUNCATE'] = fbase(
    'TRUNCATE', _truncate, NARGS_T, DT_FLOAT, RDT_FLOAT, FV_NAN)
...
def _trim(flag, ref, arg):
    ...
    if reflen == 1:
        ...
        def __trim(flag, ref, arg):
            v = arg.view('uint32').reshape(arg.size, -1)
            lp = len(ref[0])
            if v.shape[1] < lp:
                return arg
            if flag == 'TRAILING':
                arg = np.array([x[::-1] for x in arg.tolist()])
                v = arg.view('uint32').reshape(arg.size, -1)
                ref[0] = ref[0][::-1]
            p = ref.view('uint32')
            ...
            if flag == 'TRAILING':
                it = (a[s:][::-1] for a, s in zip(arg.tolist(), sidx.tolist()))
            else:
                it = (a[s:] for a, s in zip(arg.tolist(), sidx.tolist()))

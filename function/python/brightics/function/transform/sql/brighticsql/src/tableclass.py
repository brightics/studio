...
from .utils.dtype_util import (
    infer_dtype, dtypes_compatible, supertype_of_all)
from .utils.group_util import (
    get_distinct_elem, group_index, array_groupby)
...
class QueryTable(TableClass):
    ...
    def __init__(self, dataset, fields=None, dtypes=None):
        ...
        if isinstance(dataset, pd.DataFrame):
            ...
            self.fields = dataset.columns.tolist()
        elif isinstance(dataset, np.ndarray):
            assert fields is not None and len(fields) == dataset.shape[1]
            self.dstruct = 'np'
            self.shape = dataset.shape
            self.fields = fields
        elif isinstance(dataset, list):
            assert fields is not None and len(dataset) == len(fields)
            self.dstruct = 'li'
            if dataset:
                self.shape = (max([col.shape[0] for col in dataset]),
                              len(dataset))
            else:
                self.shape = (0, 0)
            self.fields = fields
    ...
    def field_index(self):
        ...
        try:
            getattr(self, '_field_index')
        except AttributeError:
            setattr(self, '_field_index',
                    dict((f, i) for i, f in enumerate(self.fields)))
    ...
    def dtypes(self):
        if len(self._dtypes) < self.shape[1]:
            for i in range(self.shape[1]):
                if not i in self._dtypes:
                    self._dtypes[i] = infer_dtype(self.slice_col(i))
    ...
    def slice_cols(self, colidx=None, totable=False):
        """ returns columns correspod to given indices.
        ...
        if not set(colidx) <= set(range(self.shape[1])):
            raise ValueError('input index contains invalid field_index')
        if self.dstruct == 'pd':
    ...
    def get_dtypes(self, colidx=None):
        ...
        if isinstance(colidx, int):
            if colidx not in self._dtypes:
                self._dtypes[colidx] = infer_dtype(self.slice_col(colidx))
    ...
    def toDataFrame(self):
        ...
        if self.dstruct == 'pd':
            ...
            self.data = [np.full(self.shape[0], c) if c.shape[0]
                         < self.shape[0] else c for c in self.data]
    ...
    def slice_rows(self, rowidx=None, totable=False):
        """ returns rows correspod to given indices.
    ...
    def append(self, other, check_compatibility=True):
        ...
        if check_compatibility:
            if not dtypes_compatible(self.dtypes, other.dtypes):
                raise Exception('Incompatible date types.')

        if self.dstruct == other.dstruct == 'pd':
            other.data.columsn = self.data.columns

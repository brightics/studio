...
def _gaussian_mixture_predict(table, model, display_probability=False, prediction_col_name='prediction'):
    
    out_table = table.copy()
    _, inputarr = check_col_type(table, model['input_cols'])
    out_table[prediction_col_name] = model['gmm'].predict(inputarr)
    if display_probability is True:
...
def gaussian_mixture_train2(table, group_by=None, **params):
    check_required_parameters(_gaussian_mixture_train2, params, ['table'])
    params = get_default_from_parameters_if_required(params, _gaussian_mixture_train2)
    param_validation_check = [greater_than_or_equal_to(params, 1, 'number_of_components'),
                              greater_than(params, 0, 'tolerance'),
                              greater_than(params, 0, 'regularize_covariance'),
                              greater_than_or_equal_to(params, 1, 'max_iteration')]
    validate(*param_validation_check)
    if group_by is not None:
        return _function_by_group(_gaussian_mixture_train2, table, group_by=group_by, **params)
    else:
        return _gaussian_mixture_train2(table, **params)


def _gaussian_mixture_train2(table, input_cols, number_of_components=1, prediction_col='prediction', covariance_type='full', tolerance=0.001, \
                            regularize_covariance=1e-06, max_iteration=100, initial_params='kmeans', seed=None):
    ...
    feature_names, X_train = check_col_type(table, input_cols)
    gmm.fit(X_train)
    gmm_prediction = gmm.predict(X_train)
    ...
    tmp_out_table = table.copy()
    tmp_out_table[prediction_col] = gmm_prediction

    return {'out_table':tmp_out_table, 'model':model}
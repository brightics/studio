...
def fasttext_model(table, model, **params):
    check_required_parameters(_fasttext_model, params, ['table', 'model'])
    return _fasttext_model(table, model, **params)


def _fasttext_model(table, model):
    doc = table[model['params']['Input column']]
    ft_model = model['ft']
    num_features = model['params']['Word vector dimensionality']

    out_table = table.copy()
    out_table['feature_vectors'] = _avg_feature_vecs(doc, ft_model, num_features).tolist()
...
def _fasttext_similarity(table, model, positive_col=None, negative_col=None, topn=1):
    ...
    if positive_col is None:
        res = df_dropped.apply(lambda _: _ft_sim_neg(ft_model, negative=_[negative_col], topn=topn), axis=1)
    elif negative_col is None:
        res = df_dropped.apply(lambda _: _ft_sim_pos(ft_model, positive=_[positive_col], topn=topn), axis=1)
    else:
        res = df_dropped.apply(
            lambda _: _ft_sim_both(ft_model, positive=_[positive_col], negative=_[negative_col], topn=topn), axis=1)
...
def _ft_sim_pos(model, positive, topn=1):
    split_positive = _split_token(positive)
    result = model.most_similar(positive=split_positive, topn=topn)
    res = [[positive] + list(result[i]) for i in range(len(result))]
    return res


def _ft_sim_neg(model, negative, topn=1):
    split_negative = _split_token(negative)
    result = model.most_similar(negative=split_negative, topn=topn)
    res = [[negative] + list(result[i]) for i in range(len(result))]
    return res


def _ft_sim_both(model, positive=None, negative=None, topn=1):
    split_positive = _split_token(positive)
    split_negative = _split_token(negative)
    result = model.most_similar(positive=split_positive, negative=split_negative, topn=topn)
    if positive is None:

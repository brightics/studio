...
def _word2vec(table, input_col, sg=1, size=100, window=5, min_count=1,
    ...
    analogy_path = data_path / 'word2vec_questions_words.txt'
    wordsim_path = data_path / 'word2vec_wordsim353.tsv'
    simlex_path = data_path / 'word2vec_simlex999.tsv'

    analogies_score, sections = w2v.wv.evaluate_word_analogies(analogy_path)
    pearson_1, spearman_1, oov_ratio_1 = w2v.wv.evaluate_word_pairs(wordsim_path)
    pearson_2, spearman_2, oov_ratio_2 = w2v.wv.evaluate_word_pairs(simlex_path)
...
def _word2vec_similarity2(table, model, positive_col=None, negative_col=None, topn=1):
    ...
    if positive_col is not None and negative_col is not None and positive_col == negative_col:
        raise ValueError('Only one of parameter can be entered between Positive Column and Negative Column')

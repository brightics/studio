...
def _result_table(input_cols, alternatives, result_dict):
    out_cols = ['data', 'alternative_hypothesis', 'statistics', 't_value',
                'p_value', 'confidence_level', 'lower_confidence_interval', 'upper_confidence_interval',
                'confidence_interval']
...
def _one_sample_ttest_repr(statistics, result_dict, params):
    ...
    for input_col in input_cols:
        ...
        #     ['alternative_hypothesis', H1_list],
        #     ['p_value', p_list],
        ...
        |
        | {result_table}
        """.format(input_col=input_col, t_value=result_dict[input_col]['t_value'],
...
def _one_sample_ttest(table, input_cols, alternatives, hypothesized_mean=0, conf_level=0.95):
    ...
    for input_col in input_cols:
        ...
        for alter in alternatives:
            (H1, p_value, lower_conf_interval, upper_conf_interval) = _test_result(alter, hypothesized_mean, mean,
                                                                                   t_value, p_value_two,
                                                                                   width_one_sided, width_two_sided)
            confidence_interval = '({lower_conf_interval}, {upper_conf_interval})'.format(
                lower_conf_interval=lower_conf_interval, upper_conf_interval=upper_conf_interval)
...
def _two_sample_ttest_for_stacked_data(table, response_cols, factor_col, alternatives, first=None, second=None,
                                       hypo_diff=0, equal_vari='pooled', confi_level=0.95):
    ...
    for response_col in response_cols:
        ...
        if equal_vari == 'auto':
            ...
            if f_test_p_value < 0.05:
                ...
                equal_vari = 'pooled'
        ttestresult = ttest_ind(table_first[response_col], table_second[response_col], 'larger', usevar=equal_vari,
                                value=hypo_diff)
        ...
        if 'larger' in alternatives:
            ...
            if equal_vari == 'pooled':
                std_number1number2 = sqrt(
                    ((number1 - 1) * (std1) ** 2 + (number2 - 1) * (std2) ** 2) / (number1 + number2 - 2))
        ...
        if 'smaller' in alternatives:
            ttestresult = ttest_ind(table_first[response_col], table_second[response_col], 'smaller', usevar=equal_vari,
                                    value=hypo_diff)
            df = ttestresult[2]
            if equal_vari == 'pooled':
                std_number1number2 = sqrt(
        ...
        if 'two-sided' in alternatives:
            ttestresult = ttest_ind(table_first[response_col], table_second[response_col], 'two-sided',
                                    usevar=equal_vari, value=hypo_diff)
            df = ttestresult[2]
            if equal_vari == 'pooled':
                std_number1number2 = sqrt(
                    ((number1 - 1) * (std1) ** 2 + (number2 - 1) * (std2) ** 2) / (number1 + number2 - 2))
            ...
            tmp_table += [['%s by %s(%s,%s)' % (response_col, factor_col, first, second)] +
                          ...
                          ['t statistic, t distribution with %f degrees of freedom under the null hypothesis' %
                           ttestresult[2]] +
                          [ttestresult[0]] + [ttestresult[1]] + [confi_level] + [mean1 - mean2 - margin] + [
                              mean1 - mean2 + margin]]

        result_model = pd.DataFrame.from_records(tmp_model)
        result_model.columns = ['alternative_hypothesis', 'p_value', 'confidence_interval']
        rb.addMD(strip_margin("""
        ...
        | {result_model}
        |
        """.format(ttestresult2=ttestresult[2], response_col=response_col, factor_col=factor_col, first=first,
                   second=second, ttestresult0=ttestresult[0], result_model=pandasDF2MD(result_model))))
        if start_auto == 1:
            equal_vari = 'auto'
    result = pd.DataFrame.from_records(tmp_table)
    result.columns = ['data', 'alternative_hypothesis', 'statistics', 'estimates', 'p_value', 'confidence_level',
...
def _paired_ttest(table, first_column, second_column, alternative, hypothesized_difference=0, confidence_level=0.95):
    ...
    if 'greater' in alternative:
        ...
        confidence_interval.append(
            (diff_mean - std_dev * stats.t.isf((1 - confidence_level), df) / np.sqrt(df), np.Infinity))

    if 'less' in alternative:
        alternative_hypothesis.append('true difference in means < ' + str(hypothesized_difference))
        p_value.append(stats.t.cdf(t_value, df))
        confidence_interval.append(
            (-np.Infinity, diff_mean + std_dev * stats.t.isf((1 - confidence_level), df) / np.sqrt(df)))

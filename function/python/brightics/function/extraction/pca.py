...
def _pca(table, input_cols, new_column_name='projected_', n_components=None, copy=True, whiten=False, svd_solver='auto',
    ...
    if n_components == 1:
        ...
        _biplot(0, 1, pc_columns=column_names, columns=input_cols, singular_values=res_singular_values,
    ...
    figs.addpltfig('plt_two', plt)
    plt.clf()

    plt.figure()
    _screeplot(res_explained_variance, res_explained_variance_ratio, n_components)
    figs.addpltfig('fig_scree', plt)
    plt.clf()
    ...
    | ### Parameters
    | {parameter1}
    """.format(image1=figs.getMD('plt_two'),
               fig_scree=figs.getMD('fig_scree'),

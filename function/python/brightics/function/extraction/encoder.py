...
def label_encoder2(table, group_by=None, **params):
    check_required_parameters(_label_encoder2, params, ['table'])
    if group_by is not None:
        grouped_model = _function_by_group(
            _label_encoder2, table, group_by=group_by, **params)
        return grouped_model
    else:
        return _label_encoder2(table, **params)
...
def label_encoder_model2(table, model, **params):
    check_required_parameters(_label_encoder_model2, params, ['table', 'model'])
    if '_grouped_data' in model:
        return _function_by_group(_label_encoder_model2, table, model, **params)
    else:
        return _label_encoder_model2(table, model, **params)


def _label_encoder_model2(table, model, suffix='_index'):
    out_table = table.copy()
    for ind, col in enumerate(model['input_cols']):
        new_col_name = col + suffix
        out_table[new_col_name] = model['label_encoders'][ind].transform(out_table[col])
    return {'out_table': out_table}
...
def one_hot_encoder2(table, group_by=None, **params):
    check_required_parameters(_one_hot_encoder2, params, ['table'])
    if group_by is not None:
        grouped_model = _function_by_group(_one_hot_encoder2, table, group_by=group_by, **params) 
        return grouped_model
    else:
...
def _one_hot_encoder2(table, input_cols, prefix='list', prefix_list=None, suffix='index', sparse=True, handle_unknown='error', drop_last=False):
    ...
    for col_name in input_cols:
        ...
        if suffix == 'index':
            ...
            for i in distinct_classes:
                ...
                if prefix == 'list':  
                    new_col_names.append(prefix_list[prefix_list_index] + '_' + i)
                else:  
    ...
    summary_table['No. distinct classes'] = number_distinct_classes
    if drop_last:
        summary_table['New encoded columns'] = new_col_names_list_with_true_drop_last
    else:

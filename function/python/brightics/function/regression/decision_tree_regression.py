...
def preprocess_():
    preprocess_result = {'estimator_class': DecisionTreeRegressor,
...
def postprocess_(table, feature_cols, regressor, display_plt):
    ...
    if display_plt:
        ...
        meta = PyPlotMeta('fig_feature_importances',
                          ...
                          yticks={'ticks': range(n_features), 'labels': sorted_feature_cols})
        meta.barh(range(n_features), feature_importance[indices], color='b', align='center')
        for i, v in enumerate(feature_importance[indices]):
            meta.text(v, i, " {:.2f}".format(v), color='b', va='center', fontweight='bold')
        meta.tight_layout()
        ...
        postprocess_result = {'update_md': {'Decision Tree': figs.getMD('fig_tree'),
                              ...
                              'type': 'decision_tree_regression_model',
                              'figs': figs}
    else:
        postprocess_result = {'type': 'decision_tree_regression_model'}
    return postprocess_result


def _decision_tree_regression_train(table, feature_cols, label_col, display_plt=True, # fig_size=np.array([6.4, 4.8]),
                                    criterion='mse', splitter='best', max_depth=None, min_samples_split=2,
                                    min_samples_leaf=1, min_weight_fraction_leaf=0.0, max_features=None,
                                    random_state=None, max_leaf_nodes=None, min_impurity_decrease=0.0,
                                    min_impurity_split=None, presort=False, sample_weight=None, check_input=True,
                                    X_idx_sorted=None):
    ...
    feature_names, features = check_col_type(table,feature_cols)
    regressor.fit(features, table[label_col], sample_weight, check_input, X_idx_sorted)

    postprocess_result = postprocess_(table, feature_cols, regressor, display_plt)
...
def _decision_tree_regression_predict(table, model, prediction_col='prediction', check_input=True):

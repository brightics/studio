...
def _predict(ratings, similar_coeff, target, k, weighted):
    ...
    modi_best = list(filter(lambda x:x[1][0] > 0 and x[1][1] != 0, best))
    if len(modi_best) < k:
        return 0
    top = modi_best[0:k]
    if weighted:
...
def _collaborative_filtering_train(table, user_col , item_col, rating_col, N=10, k=5, based = 'item', mode='train', method = 'cosine', weighted = True, centered = True, targets = None):
...
def _collaborative_filtering_recommend(table, user_col , item_col, rating_col, N=10, k=5, based = 'item', mode='Topn', method = 'cosine', weighted = True, centered = True, targets = None):
    return _collaborative_filtering_train(table, user_col , item_col, rating_col, N, k, based, mode, method, weighted, centered, targets)

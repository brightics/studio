            ...
            try:
                ...
                for i in code_tree.body:
                    if isinstance(i, ast.FunctionDef) or isinstance(i, ast.ClassDef):
                        ...
                        other_code.append(i)
                exec_code = other_code[:-1]
                single_code = other_code[-1:]  # last node of code
                def_object = compile(ast.Module(def_code), '<string>', 'exec')
    ...
    try:
        ...
        while s and s.rstrip() not in ['exit()', 'quit']:
            try:
                ...
                if brtc_java_gateway.logger.isDebugEnabled():
                ...
                #brtc_java_gateway.notify_python_process_finished(traceback.format_exc(), True, json.dumps(bfe.errors))

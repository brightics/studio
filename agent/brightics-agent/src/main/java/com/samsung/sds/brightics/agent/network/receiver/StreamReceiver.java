...
public class StreamReceiver extends StreamServiceGrpc.StreamServiceImplBase {
    ...
    @Override
    public void readCheckpoint(ReadCheckpointMessage message, StreamObserver<ByteStreamMessage> responseObserver) {
        String key = listener.receive(message);
        ThreadLocalContext.put("user", message.getUser());
        logger.info("[Common network] receive read checkpoint : " + message.toString() + ", user : " + message.getUser());

        StreamService.readCheckpoint(message, responseObserver);

        listener.onCompleted(key);
    }

    @Override
    public void download(DownloadMessage request, StreamObserver<ResultMessage> responseObserver) {

        String key = listener.receive(request);
        ThreadLocalContext.put("user", request.getUser());
        logger.info("[Common network] download : " + request.toString() + ", user : " + request.getUser());

        ResultMessage resultMessage = StreamService.download(request);
